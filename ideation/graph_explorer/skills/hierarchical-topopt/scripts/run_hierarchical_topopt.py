    ...
    for y in range(nrows):
        for x in range(ncols):
            ...
            if height <= 1e-12:
                continue
            active += 1
            triangles.extend(box_triangles(float(x), float(y), 0.0, 1.0, 1.0, height))
    write_ascii_stl(path, triangles, name)
    return {"path": path.name, "active_cells": active, "triangles": len(triangles)}
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if mode in {"profile-stl", "all-basic"}:
        mesh_outputs.append(voxel_stl_from_density(resized, out / "result_profile.stl", "result_profile", args.max_height, args.dens_cut, 1.0, True))
